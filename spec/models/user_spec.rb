...
RSpec.describe User, type: :model do
  ...
  describe '#check_all_events_finished' do
    ...
    context 'ユーザに関連する公開中の未終了イベント、未終了の参加イベントともに存在していないとき' do
      ...
      it 'nil が返ること' do
        expect(user.send(:check_all_events_finished)).to eq nil
      end
    ...
    context '公開中の未終了イベントが存在するとき' do
      ...
      it '例外 が発生すること' do
        expect{ user.send(:check_all_events_finished) }.to raise_error(UncaughtThrowError)
      end

      # TODO: 例外発生時のパラメータのテスト
      xit '#errors[:base] に "公開中の未終了イベントが存在します。" が格納されていること' do
        user.send(:check_all_events_finished)
    ...
    context '未終了の参加イベントが存在するとき' do
      ...
      # TODO: 例外発生時のパラメータのテスト
      xit '#errors[:base] に "未終了の参加イベントが存在します。" が格納されていること' do
        user.send(:check_all_events_finished)
